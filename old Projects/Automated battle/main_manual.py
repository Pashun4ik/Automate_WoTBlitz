...
kernel_distance = np.ones((5, 5), np.uint8)
...
while 1:
    ...
    # Distance counter frame
    frameBGR_distance = CropAndResize(frameBGR_mon,
                                      int(mon_height * 0.50),
                                      int(mon_height * 0.52),
                                      int(mon_width * 0.52),
                                      int(mon_width * 0.5688),
                                      300,
                                      100).new_frame_bgr

    frameGray_distance = cv2.cvtColor(frameBGR_distance, cv2.COLOR_BGR2GRAY, 10, 8)
    frameDilate_distance = cv2.dilate(frameGray_distance, kernel_distance, iterations=1)
    frameEroded_distance = cv2.erode(frameDilate_distance, kernel_distance, iterations=1)
    frameCanny_distance = cv2.Canny(frameEroded_distance, 100, 100)
    distance = pytesseract.image_to_string(frameEroded_distance)
    print(distance.replace('m', ''))
    ...
    if d_x != mon_width / 2 and d_y != mon_height / 2:
        ...
        try:
            if distance != int:
                distance = 300
            elif distance < 50 and aimed:
                offset_y = 300
            elif distance < 50 and not aimed:
                offset_y = 100
            elif distance > 100:
                offset_y = 300
            elif distance > 200:
    ...
    detect_bright = detect_object(modify_bright.dilated_mask, modify_bright.dilated_contours, 500, 8)
    if detect_bright:
    ...
    # cv2.imshow('Distance counter', frameEroded_distance)
    # cv2.imshow('CV', modify_CV_view.frame_result)
